...
module ForestVPNCloudAPI
  class CreateOrUpdatePaymentProfileRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ForestVPNCloudAPI::CreateOrUpdatePaymentProfileRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ForestVPNCloudAPI::CreateOrUpdatePaymentProfileRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @address_line3.nil?
        invalid_properties.push('invalid value for "address_line3", address_line3 cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @address_line3.nil?
    ...
    def hash
      [tax_number, business_name, name, address_line1, address_line2, address_line3, primary_contact_name, primary_contact_phone, primary_contact_email].hash
    end
